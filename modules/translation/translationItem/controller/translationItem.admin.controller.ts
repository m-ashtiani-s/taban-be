...
export class TranslationAdminController extends ControllerBase {
	...
	deleteTranslationItem = async (req: Request, res: Response) => {
		const errors = validationResult(req);
		if (!errors.isEmpty()) {
			return this.showValidationErrors(res, errors);
		}
		...
		try {
			const translationItemId: string = req.params.translationItemId ?? "";
			const result = await translationService.deleteTranslationItem(translationItemId);
			return res.status(200).json(result);
		} catch (error: ControllerError) {
			...
			return res.status(statusCode).json({
				...
				success: false,
				data: null,
				message: error.message || "مشکلی در حذف مدرک رخ داد",
			});
		}
	};
}
